...
import { STRUCTURE_CONTRACT } from "./structure-contract.js";
import {
...
// Runs the declarative structural contract (file-exists / source-substring
// assertions) lifted out of the inline runLocalEvals() array. Keeping the
// file<->substring couplings in one table means renames/splits edit one place.
function runStructureContract(root: string): EvalCaseResult[] {
  return STRUCTURE_CONTRACT.map((entry) => {
    if (entry.kind === "file") {
      return checkFile(root, entry.path, entry.name);
    }
    if (entry.kind === "contains") {
      return checkContains(root, entry.path, entry.needle, entry.name);
    }
    return checkNotContains(root, entry.path, entry.needle, entry.name);
  });
}

export function runLocalEvals(root = process.cwd()): EvalCaseResult[] {
  return [
    ...runStructureContract(root),
