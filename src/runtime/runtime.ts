...
import { delegatedToolStatus, sessionStatusFromExecutionStatus, workerStatusFromExecutionStatus } from "./execution-status.js";
...
export class SwarmRuntime {
  ...
  private readonly runtimeEventPersister: RuntimeEventPersister;
  ...
  constructor(options: { databasePath?: string; workspace?: string; approvalHandler?: ToolApprovalHandler; debugSessionId?: string; mcpConfig?: RuntimeMcpConfigOptions } = {}) {
    ...
    this.runtimeEventPersister = new RuntimeEventPersister({
      taskGraphStore: this.taskGraphStore,
      approvalStore: this.approvalStore,
      sessionStore: this.sessionStore,
      usageStore: this.usageStore,
      auditStore: this.auditStore,
      sessionContextStore: this.sessionContextStore,
      runAttemptStore: this.runAttemptStore,
      router: this.router,
      workspaceForSession: (sessionId) => this.workspaceForSession(sessionId),
      activeSessionId: () => this.activeCodingLoopSessionId ?? this.activeSwarmSession?.session_id,
      recordProviderUsage: (usage) => this.recordProviderUsage(usage),
      isDisposed: () => this.disposed,
      logger: this.debug ?? undefined
    });
    this.unsubscribeRuntimeEventPersister = this.events.onEvent((event) => this.runtimeEventPersister.record(event));
